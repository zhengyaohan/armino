        ...
        ValSize_t * const VersionSize_p)
{
    ...
    static const char gl_APIVersion[] = "VAL APIv" VAL_API_VERSION;
    ...
    if (funcres == VAL_SUCCESS)
    {
        if (t_res.Result == VEXTOKEN_RESULT_NO_ERROR)
        {
            ...
            *ptr++ = ' ';
            ...
            ptr[4]  = (uint8_t)(ptr[4] + t_res.Service.SystemInfo.Hardware.Major);
            ptr[6]  = (uint8_t)(ptr[6] + t_res.Service.SystemInfo.Hardware.Minor);
            ptr[8]  = (uint8_t)(ptr[8] + t_res.Service.SystemInfo.Hardware.Patch);
            ptr[13] = (uint8_t)(ptr[13] + t_res.Service.SystemInfo.Firmware.Major);
            ptr[15] = (uint8_t)(ptr[15] + t_res.Service.SystemInfo.Firmware.Minor);
            ptr[17] = (uint8_t)(ptr[17] + t_res.Service.SystemInfo.Firmware.Patch);
            ...
            for (i = 3; i >= 0; i--)
            {
                ...
                if (Value <= 9)
                {
                    *ptr = (uint8_t)(*ptr + Value);
                }
                else
                {
                    *ptr = (uint8_t)(*ptr + Value + 7);
                }
        ...
        uint32_t * const Identity_p)
{
    ...
    if (funcres == VAL_SUCCESS)
    {
        if (t_res.Result == VEXTOKEN_RESULT_NO_ERROR)
        {
            ...
            if (SelfTestError_p != NULL)
            {
                *SelfTestError_p = t_res.Service.SystemInfo.Self.ErrorTest;
            }
 ...
 * val_SystemSetTime
 */
#ifndef VAL_REMOVE_SYSTEM_SET_TIME
ValStatus_t
val_SystemSetTime(
        const uint32_t Seconds)
{
    ...
    // Format service request
    t_cmd.OpCode  = VEXTOKEN_OPCODE_SYSTEM;
    t_cmd.SubCode = VEXTOKEN_SUBCODE_SETTIME;
    t_cmd.Service.SystemSetTime.Seconds = Seconds;
    t_res.Result = VEXTOKEN_RESULT_NO_ERROR;
...
#endif /* !VAL_REMOVE_SYSTEM_SET_TIME */
...
val_IsAccessSecure(void)
{
    ...
    if ((gl_ValAccessType != 0x55AA55) && (gl_ValAccessType != 0x882288))
    {
        ...
        FuncRes = val_SystemGetState(NULL, NULL, NULL, NULL, NULL, NULL, &NonSecure, NULL);
        if(FuncRes == VAL_SUCCESS)
        {
            if (NonSecure)
            {
