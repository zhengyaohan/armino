...
This readme describes the EIP-13x DDK Regression System provided with
the EIP-13x Driver Development Kit.
...
- Copy the BOOT.bin file to the SD card and boot the Zynq ZC702 FPGA board
  ...
  application, test tool and EIP-13x driver built for a specific configuration.
  The FPGA bitstream and the first stage boot loader are located in
  the MicroBlaze Board Support Package (BSP) that can be obtained on request
  from INSIDE Secure or built spearately outside of the DDK using the Xilinx
  SDK.
