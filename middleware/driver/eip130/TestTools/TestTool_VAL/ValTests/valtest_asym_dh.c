        ...
        bool ftestExport)
{
    ...
    Status = val_AsymDhDsaAllocDomainAsset(Domain.PrimeSizeBits,
                                           ...
                                           DomainAssetId_p);
    fail_if(Status != VAL_SUCCESS, "val_AsymDhDsaAllocDomainAsset()=", Status);
    ...
    if (KEKAssetId != VAL_ASSETID_INVALID)
    {
        ...
        fail_if(Status != VAL_BUFFER_TOO_SMALL, "val_AsymDhDsaLoadDomainAssetPlaintext()=", Status);
        ...
        fail_if(Status != VAL_SUCCESS, "val_AsymDhDsaLoadDomainAssetPlaintext()=", Status);
        ...
        bool fSaveSharedSecret)
{
    ...
    Status = val_AsymDhAllocPrivateKeyAsset(DivisorSizeBits, false, false, &SndPrivateKeyAssetId);
    fail_if(Status != VAL_SUCCESS, "val_AsymDhAllocPrivateKeyAsset(Sender)=", Status);
    ...
    fail_if(Status != VAL_SUCCESS, "val_AsymDhAllocPublicKeyAsset(Sender)=", Status);
    ...
    fail_if(Status != VAL_SUCCESS, "val_AsymDhGenKeyPair(Sender)=", Status);

    /* Receiver */
    /* - Allocate the private key asset and initialize key structure */
    Status = val_AsymDhAllocPrivateKeyAsset(DivisorSizeBits, false, false, &RcvPrivateKeyAssetId);
    fail_if(Status != VAL_SUCCESS, "val_AsymDhAllocPrivateKeyAsset(Receiver)=", Status);
    ...
    fail_if(Status != VAL_SUCCESS, "val_AsymDhAllocPublicKeyAsset(Receiver)=", Status);
    ...
    Status = val_AsymDhGenKeyPair(&RcvPublicKey, &RcvPrivateKey,
                                  VAL_ASSETID_INVALID, NULL, 0, NULL, NULL, NULL);
    fail_if(Status != VAL_SUCCESS, "val_AsymDhGenKeyPair(Receiver)=", Status);

    /* Check keys of receiver only (API coverage) */
    Status = val_AsymDhDsaKeyCheck(&RcvPublicKey, &RcvPrivateKey);
    fail_if(Status != VAL_SUCCESS, "val_AsymDhDsaKeyCheck(Both)=", Status);
    ...
    fail_if(Status != VAL_SUCCESS, "val_AsymDhAllocPublicKeyAsset(Test)=", Status);
    ...
    fail_if(Status != VAL_SUCCESS, "val_AsymDhGenPublicKey(Test)=", Status);

    Status = val_AsymDhDsaKeyCheck(&TestPublicKey, &RcvPrivateKey);
    fail_if(Status != VAL_SUCCESS, "val_AsymDhDsaKeyCheck(Test)=", Status);

    Status = val_AssetFree(TestPublicKeyAssetId);
    fail_if(Status != VAL_SUCCESS, "val_AssetFree(Test)=", Status);
    ...
    if (fDual)
    {
        ...
        /* Sender */
        /* - Allocate the private key asset and initialize key structure */
        Status = val_AsymDhAllocPrivateKeyAsset(DivisorSizeBits, false, false, &SndPrivateKey2AssetId);
        fail_if(Status != VAL_SUCCESS, "val_AsymDhAllocPrivateKeyAsset(Sender2)=", Status);
        ...
        fail_if(Status != VAL_SUCCESS, "val_AsymDhAllocPublicKeyAsset(Sender2)=", Status);
        ...
        Status = val_AsymDhGenKeyPair(&SndPublicKey2, &SndPrivateKey2,
                                       VAL_ASSETID_INVALID, NULL, 0, NULL, NULL, NULL);
        fail_if(Status != VAL_SUCCESS, "val_AsymDhGenKeyPair(Sender2)=", Status);

        /* Receiver */
        /* - Allocate the private key asset and initialize key structure */
        Status = val_AsymDhAllocPrivateKeyAsset(DivisorSizeBits, false, false, &RcvPrivateKey2AssetId);
        fail_if(Status != VAL_SUCCESS, "val_AsymDhAllocPrivateKeyAsset(Receiver2)=", Status);
        ...
        fail_if(Status != VAL_SUCCESS, "val_AsymDhAllocPublicKeyAsset(Receiver2)=", Status);
        ...
        Status = val_AsymDhGenKeyPair(&RcvPublicKey2, &RcvPrivateKey2,
                                       VAL_ASSETID_INVALID, NULL, 0, NULL, NULL, NULL);
        fail_if(Status != VAL_SUCCESS, "val_AsymDhGenKeyPair(Receiver2)=", Status);
    }
    ...
    if (fSaveSharedSecret)
    {
        ...
        if (DeriveKeySize > 64)
        {
            AssetPolicy = VAL_POLICY_KEY_DERIVE | VAL_POLICY_SHA512;
        }
        else
        {
    ...
    if (fDual)
    {
        ...
        fail_if(Status != VAL_SUCCESS, "val_AsymDhGenSharedSecretDual(Sender)=", Status);
        ...
        fail_if(Status != VAL_SUCCESS, "val_AsymDhGenSharedSecretDual(Receiver)=", Status);
    }
    else
    {
        ...
        fail_if(Status != VAL_SUCCESS, "val_AsymDhGenSharedSecretSingle(Sender)=", Status);
        Status = val_AsymDhGenSharedSecretSingle(&SndPublicKey, &RcvPrivateKey,
                                                 fSaveSharedSecret,
                                                 Info_p, InfoSize,
                                                 RcvAssetList, (ValSize_t)ListAssets);
        fail_if(Status != VAL_SUCCESS, "val_AsymDhGenSharedSecretSingle(Receiver)=", Status);
    }
...
START_TEST(test_dh_GenerateSharedSecret_L1024N160)
{
    ...
    rc = test_dh_GenerateSharedSecret(DhDsaDomain_L1024N160_p,
                                     ((_i & 1) != 0), ((_i & 2) != 0));
    if (rc != END_TEST_SUCCES)
    {
        return rc;
...
START_TEST(test_dh_GenerateSharedSecret_L2048N224)
{
    ...
    rc = test_dh_GenerateSharedSecret(DhDsaDomain_L2048N224_p,
                                     ((_i & 1) != 0), ((_i & 2) != 0));
    if (rc != END_TEST_SUCCES)
    {
        return rc;
...
START_TEST(test_dh_GenerateSharedSecret_L2048N256)
{
    ...
    rc = test_dh_GenerateSharedSecret(DhDsaDomain_L2048N256_p,
                                     ((_i & 1) != 0), ((_i & 2) != 0));
    if (rc != END_TEST_SUCCES)
    {
        return rc;
...
START_TEST(test_dh_GenerateSharedSecret_L3072N256)
{
    ...
    rc = test_dh_GenerateSharedSecret(DhDsaDomain_L3072N256_p,
                                     ((_i & 1) != 0), ((_i & 2) != 0));
    if (rc != END_TEST_SUCCES)
    {
        return rc;
