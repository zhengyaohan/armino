...
START_TEST(test_AsymPkModExpE)
{
    ...
    fail_if(Status != VAL_SUCCESS, "val_ClaimOverrule()=", Status);
...
START_TEST(test_AsymPkDsaSign)
{
    ...
    ModVectorSize = (sizeof(n) + 3) & (ValSize_t)~3;
    ExpVectorSize = (sizeof(p) + 3) & (ValSize_t)~3;
    nWords = (ModVectorSize / 4);
    mWords = (ExpVectorSize / 4);
    BufferWordsSize = (2 + (nWords & 1)) * 4;

    Status = val_AsymPkaClaim((uint8_t)nWords, (uint8_t)mWords, 0x3); // first two vectors (p & g) are long
    fail_if(Status != VAL_SUCCESS, "val_AsymPkaClaim()=", Status);

    Vector_p = AllocAndInitializeVector(p, sizeof(p), (uint32_t)ExpVectorSize, true);
    fail_if(Vector_p == NULL, "Allocation ", (int)ExpVectorSize);
    Status = val_AsymPkaLoadVector(0, Vector_p, ExpVectorSize);
    SFZUTF_FREE(Vector_p);
    fail_if(Status != VAL_SUCCESS, "val_AsymPkaLoadVector(p)=", Status);

    Vector_p = AllocAndInitializeVector(g, sizeof(g), (uint32_t)ExpVectorSize, true);
    fail_if(Vector_p == NULL, "Allocation ", (int)ExpVectorSize);
    Status = val_AsymPkaLoadVector(1, Vector_p, ExpVectorSize);
    SFZUTF_FREE(Vector_p);
    fail_if(Status != VAL_SUCCESS, "val_AsymPkaLoadVector(g)=", Status);

    Vector_p = AllocAndInitializeVector(n, sizeof(n), (uint32_t)ModVectorSize, true);
    ...
    Vector_p = AllocAndInitializeVector(h, sizeof(h), (uint32_t)ModVectorSize, true);
    fail_if(Vector_p == NULL, "Allocation ", (int)ModVectorSize);
    Status = val_AsymPkaLoadVector(3, Vector_p, ModVectorSize);
    SFZUTF_FREE(Vector_p);
    fail_if(Status != VAL_SUCCESS, "val_AsymPkaLoadVector(k)=", Status);

    Vector_p = AllocAndInitializeVector(k, sizeof(k), (uint32_t)ModVectorSize, true);
    fail_if(Vector_p == NULL, "Allocation ", (int)ModVectorSize);
    Status = val_AsymPkaLoadVector(4, Vector_p, ModVectorSize);
    SFZUTF_FREE(Vector_p);
    fail_if(Status != VAL_SUCCESS, "val_AsymPkaLoadVector(k)=", Status);

    Vector_p = AllocAndInitializeVector(y, sizeof(y), (uint32_t)ModVectorSize, true);
    fail_if(Vector_p == NULL, "Allocation ", (int)ModVectorSize);
    ...
    Status = val_AsymPkaExecuteOperation(VAL_ASYM_PKA_OP_DSASIGN, 0,
                                         ...
                                         Result_p, &ResultSize);
    fail_if(Status != VAL_SUCCESS, "val_AsymPkaExecuteOperation()=", Status);
    fail_if(CheckResultVector(Result_p, (uint32_t)ModVectorSize, ro, sizeof(ro), true) == false, "", Status);
    fail_if(CheckResultVector(&Result_p[ModVectorSize + BufferWordsSize], (uint32_t)ModVectorSize, so, sizeof(so), true) == false, "", Status);
...
START_TEST(test_AsymPkDsaVerify)
{
    ...
    Vector_p = AllocAndInitializeVector(ri, sizeof(ri), (uint32_t)ModVectorSize, true);
    fail_if(Vector_p == NULL, "Allocation ", (int)ModVectorSize);
    Status = val_AsymPkaLoadVector(4, Vector_p, ModVectorSize);
    SFZUTF_FREE(Vector_p);
    fail_if(Status != VAL_SUCCESS, "val_AsymPkaLoadVector(h)=", Status);

    Vector_p = AllocAndInitializeVector(si, sizeof(si), (uint32_t)ModVectorSize, true);
    fail_if(Vector_p == NULL, "Allocation ", (int)ModVectorSize);
    Status = val_AsymPkaLoadVector(5, Vector_p, ModVectorSize);
    SFZUTF_FREE(Vector_p);
    fail_if(Status != VAL_SUCCESS, "val_AsymPkaLoadVector(h)=", Status);

    Vector_p = AllocAndInitializeVector(alpha, sizeof(alpha), (uint32_t)ExpVectorSize, true);
    fail_if(Vector_p == NULL, "Allocation ", (int)ModVectorSize);
    ...
    Status = val_AsymPkaExecuteOperation(VAL_ASYM_PKA_OP_DSAVERIFY, 0,
                                         ...
                                         Result_p, &ResultSize);
    fail_if(Status != VAL_SUCCESS, "val_AsymPkaExecuteOperation()=", Status);
    fail_if(CheckResultVector(Result_p, (uint32_t)ModVectorSize, ri, sizeof(ri), true) == false, "", Status);
    fail_if(CheckResultVector(&Result_p[ModVectorSize + BufferWordsSize], (uint32_t)ModVectorSize, ri, sizeof(ri), true) == false, "", Status);
