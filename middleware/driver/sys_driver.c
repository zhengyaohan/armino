...
{
	return sys_drv_all_modules_clk_div_get(reg);
}
...
void sys_drv_dma2d_set(uint8_t clk_always_on)
{
	uint32_t int_level = rtos_disable_int();
...
uint32_t sys_drv_cpu0_i2s_int_en(uint32_t value)
{
	uint32_t int_level = rtos_disable_int();

	sys_hal_cpu0_i2s_int_en(value);
	rtos_enable_int(int_level);
	return SYS_DRV_SUCCESS;
}
