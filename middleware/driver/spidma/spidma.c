...
#define USE_SPI_DMA_GPIO_NUM        USE_SPI_DMA_GPIO_30_33
...
static void spidma_config_rxdma(void)
{
	...
	sddev_control(DD_DEV_TYPE_GDMA, CMD_GDMA_CFG_TYPE5, &cfg);

	en_cfg.channel = p_spidma_desc->dma_rx_channel;
	en_cfg.param = p_spidma_desc->node_len; // dma translen
	sddev_control(DD_DEV_TYPE_GDMA, CMD_GDMA_SET_TRANS_LENGTH, &en_cfg);
...
static void spidma_config_txdma(void)
{
	...
	sddev_control(DD_DEV_TYPE_GDMA, CMD_GDMA_CFG_TYPE4, &cfg);
	...
	//sddev_control(DD_DEV_TYPE_GDMA, CMD_GDMA_CFG_WORK_MODE, &en_cfg);
...
static void spidma_start_txdma(UINT8 *tx_buf, UINT32 tx_len)
{
	...
	sddev_control(DD_DEV_TYPE_GDMA, CMD_GDMA_SET_SRC_START_ADDR, &en_cfg);
...
{
	p_spidma_desc = NULL;
	ddev_register_dev(DD_DEV_TYPE_SPIDMA, (DD_OPERATIONS *)&spidma_op);
}
...
static void spidma_hardware_init(void)
{
	UINT32 reg;
	/* register interrupt */
	bk_int_isr_register(INT_SRC_HSSPI_SLAVE, spidma_isr, NULL);
