...
static const DD_OPERATIONS qspi_op = {
	NULL,
	NULL,
	NULL,
	NULL,
...
void qspi_init(void)
{
	REG_WRITE(REG_GPIO_6_CONFIG, 0x02);
	REG_WRITE(REG_GPIO_6_CONFIG, 0x00);

	bk_int_isr_register(INT_SRC_PSRAM, qspi_isr, NULL);
	sddev_register_dev(DD_DEV_TYPE_QSPI, (DD_OPERATIONS *)&qspi_op);
}

void qspi_exit(void)
{
	sddev_unregister_dev(DD_DEV_TYPE_QSPI);
}
...
static void qspi_psram_set_voltage(UINT32 mode)
{
	...
	param = mode;
	//sddev_control(DD_DEV_TYPE_SCTRL, CMD_QSPI_IO_VOLTAGE, &param);
	sys_drv_set_qspi_io_voltage(param);
...
static void qspi_gpio_configuration(UINT8 LineMode)
{
	...
	if (LineMode == 1) {
		...
		sddev_control(DD_DEV_TYPE_GPIO, CMD_GPIO_ENABLE_SECOND, &val);
		gpio_config(30, GMODE_INPUT_PULLDOWN);
		gpio_config(31, GMODE_INPUT_PULLDOWN);
	} else if (LineMode == 4) {
		//set io0/io1
		val = GFUNC_MODE_QSPI_1LINE;
		sddev_control(DD_DEV_TYPE_GPIO, CMD_GPIO_ENABLE_SECOND, &val);
...
static void qspi_icu_configuration(UINT32 enable)
{
	...
	if (enable) {
		param = PWD_QSPI_CLK;
		//sddev_control(DD_DEV_TYPE_ICU, CMD_FUNC_CLK_PWR_UP, &param);
		sys_drv_func_clk_pwr_up(param);
		...
		param = PWD_QSPI_CLK;
		//sddev_control(DD_DEV_TYPE_ICU, CMD_FUNC_CLK_PWR_DOWN, &param);
		sys_drv_func_clk_pwr_down(param);
...
static void qspi_clk_set_26M(void)
{
	...
	//sddev_control(DD_DEV_TYPE_ICU, CMD_QSPI_CLK_SEL, &param);
	sys_drv_qspi_clk_sel(param);
}
...
void bk_qspi_psram_quad_mode_switch(unsigned char is_single_mode)
{
	...
	QSPI_DEBUG_PRINTF("frank#-----clk_mux :%x\r\n", REG_READ(ICU_PERI_CLK_MUX));
	QSPI_DEBUG_PRINTF("frank#------int_enable:%x\r\n", REG_READ(ICU_INTERRUPT_ENABLE));
	QSPI_DEBUG_PRINTF("frank#------icu_pwd:%x\r\n", REG_READ(ICU_FUNC_CLK_PWD));
	QSPI_DEBUG_PRINTF("frank#------pmu_block:%x\r\n", REG_READ(SCTRL_BLOCK_EN_CFG));
...
#define GPIO_PSRSM_CE		REG_GPIO_27_CONFIG
#define GPIO_PSRSM_CLK		REG_GPIO_26_CONFIG
#define GPIO_PSRSM_SI		REG_GPIO_28_CONFIG
#define GPIO_PSRSM_SO		REG_GPIO_29_CONFIG
...
void qspi_printf(void)
{
	QSPI_DEBUG_PRINTF("REG_QSPI_SW_CMD = 0x%lx\r\n", REG_READ(REG_QSPI_SW_CMD));
	QSPI_DEBUG_PRINTF("REG_QSPI_SW_ADDR = 0x%lx\r\n", REG_READ(REG_QSPI_SW_ADDR));
	QSPI_DEBUG_PRINTF("REG_QSPI_SW_DUM = 0x%lx\r\n", REG_READ(REG_QSPI_SW_DUM));
	QSPI_DEBUG_PRINTF("REG_QSPI_SW_DAT = 0x%lx\r\n", REG_READ(REG_QSPI_SW_DAT));
	QSPI_DEBUG_PRINTF("REG_QSPI_SW_OP = 0x%lx\r\n", REG_READ(REG_QSPI_SW_OP));
	QSPI_DEBUG_PRINTF("REG_QSPI_GE0_TH = 0x%lx\r\n", REG_READ(REG_QSPI_GE0_TH));
	QSPI_DEBUG_PRINTF("REG_QSPI_GE0_DEP = 0x%lx\r\n", REG_READ(REG_QSPI_GE0_DEP));
	QSPI_DEBUG_PRINTF("REG_QSPI_GE1_TH = 0x%lx\r\n", REG_READ(REG_QSPI_GE1_TH));
	QSPI_DEBUG_PRINTF("REG_QSPI_GE1_DEP = 0x%lx\r\n", REG_READ(REG_QSPI_GE1_DEP));
	QSPI_DEBUG_PRINTF("REG_QSPI_ENABLE = 0x%lx\r\n", REG_READ(REG_QSPI_ENABLE));
	QSPI_DEBUG_PRINTF("REG_QSPI_CONFIG = 0x%lx\r\n", REG_READ(QSPI_CTRL));
	QSPI_DEBUG_PRINTF("REG_QSPI_FIFO_STATUS = 0x%lx\r\n", REG_READ(REG_QSPI_FIFO_STATUS));
	QSPI_DEBUG_PRINTF("REG_QSPI_INT_STATUS = 0x%lx\r\n", REG_READ(REG_QSPI_INT_STATUS));
}
	...
	if (ulStatus & (1 << 16)) {  // sw op int
		...
		if (sg_p_qspi_ge0_drv_desc->mode & 0x04) {      // read from psram
			...
			//            if (sg_p_qspi_ge0_drv_desc->data_buff_offset >= sg_p_qspi_ge0_drv_desc->data_buff_size)
			{
				...
				//debug:close int
				reg = (IRQ_QSPI_BIT);
				// sddev_control(DD_DEV_TYPE_ICU, CMD_ICU_INT_DISABLE, &reg);
				(void)sys_drv_int_disable(reg);
			}
			//            else
			//            {
			//                DEBUG_QSPI_PRINTF("j\r\n");
