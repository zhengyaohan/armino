...
extern void arch_atomic_spin_set(u32 * lock_addr);
...
void init_spinlock(spinlock_t *slock)
{
...
void spinlock_acquire(spinlock_t *slock)
{
	...
	if(spinlock_owner(slock))
	{
		...
		bk_printf(" FATAL ERROR in %s", __FILE__);
	}

	arch_atomic_spin_set(&slock->locked);
...
void spinlock_release(spinlock_t *slock)
{
	...
	if(!spinlock_owner(slock))
	{
		...
		bk_printf(" FATAL ERROR2 in %s", __FILE__);
		return;
	}
