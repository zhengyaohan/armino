...
SDIO_NODE_PTR sdio_alloc_valid_node(UINT32 buf_size)
{
	...
	if (temp_node_ptr) {
		...
		temp_node_ptr->addr   = temp_node_ptr->orig_addr + CONFIG_MSDU_RESV_HEAD_LEN;
		temp_node_ptr->length = buf_size;
		mem_node_ptr = temp_node_ptr;
#else
		UINT8 *buff_ptr = (UINT8 *)os_malloc(CONFIG_MSDU_RESV_HEAD_LEN + buf_size
		...
		if (buff_ptr) {
			...
			temp_node_ptr->addr   = (UINT8 *)((UINT32)buff_ptr + CONFIG_MSDU_RESV_HEAD_LEN);
			temp_node_ptr->length = buf_size;
