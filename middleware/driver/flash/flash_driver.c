...
static uint32_t hold_low_speed_status = 0;
#endif
...
bk_err_t bk_flash_clk_switch(uint32_t flash_speed_type, uint32_t modules)
{
	...
	switch (flash_speed_type) {
		case FLASH_SPEED_LOW:
			hold_low_speed_status |= modules;
			FLASH_LOGI("%s: set low, 0x%x 0x%x\r\n", __func__, hold_low_speed_status, modules);
	
			if (hold_low_speed_status) {
		...
		case FLASH_SPEED_HIGH:
			hold_low_speed_status &= ~(modules);
			FLASH_LOGI("%s: clear low bit, 0x%x 0x%x\r\n", __func__, hold_low_speed_status, modules);
	
			if (0 == hold_low_speed_status) {
