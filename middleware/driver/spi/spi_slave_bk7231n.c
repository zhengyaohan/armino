...
#if(CONFIG_SOC_BK7231N) || (CONFIG_SOC_BK7256)
...
static void bk_spi_slave_tx_needwrite_callback(int port, void *param)
{
	...
	if (tx_ptr && tx_len) {
		...
		while (spi_write_txfifo(data) == 1) {
			...
			if (tx_len == 0) {
				UINT32 enable = 0;
				sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_TXINT_EN, (void *)&enable);
				break;
			}
			data = *tx_ptr;
		}
...
static void bk_spi_slave_configure(UINT32 rate, UINT32 mode)
{
	...
	/* data bit width */
	param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_SET_BITWIDTH, (void *)&param);

	/* baudrate */
	BK_SPI_PRT("max_hz = %d \n", rate);
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_SET_CKR, (void *)&rate);
	...
	if (mode & BK_SPI_CPOL)
		...
		param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_SET_CKPOL, (void *)&param);
	...
	if (mode & BK_SPI_CPHA)
		...
		param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_SET_CKPHA, (void *)&param);

	/* slave */
	param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_SET_MSTEN, (void *)&param);

	// 4line :7231N nssms is 0
	param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_SET_NSSMD, (void *)&param);

	param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_INIT_MSTEN, (void *)&param);


	/* set call back func */
	spi_dev_cb.callback = bk_spi_slave_spi_rx_callback;
	spi_dev_cb.param = NULL;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_SET_RX_CALLBACK, (void *)&spi_dev_cb);

	spi_dev_cb.callback = bk_spi_slave_tx_needwrite_callback;
	spi_dev_cb.param = NULL;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_SET_TX_NEED_WRITE_CALLBACK, (void *)&spi_dev_cb);

	spi_dev_cb.callback = bk_spi_slave_tx_finish_callback;
	spi_dev_cb.param = NULL;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_SET_TX_FINISH_CALLBACK, (void *)&spi_dev_cb);

	param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_LSB_EN, (void *)&param);

	//enable rx/tx finish enable bit
	param = 1;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_TXFINISH_INT_EN, (void *)&param);

	param = 1;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_RXFINISH_INT_EN, (void *)&param);
	...
	param = 1;
#endif
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_RXINT_EN, (void *)&param);

	/* enable spi */
	param = 1;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_UNIT_ENABLE, (void *)&param);
...
{
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_DEINIT_MSTEN, NULL);
}
...
int bk_spi_slave_xfer(struct spi_message *msg)
{
	...
	param = send_len - 1;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_TXTRANS_LEN, (void *)&param);

	param = recv_len;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_RXTRANS_LEN, (void *)&param);

	//enbale rx/tx enable bit
	param = 1;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_TX_EN, (void *)&param);

	param = 1;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_RX_EN, (void *)&param);
	...
	if ((send_ptr) && send_len) {
		...
		param = 1;
		sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_TXINT_EN, (void *)&param);
...
void spi_dma_rx_init(struct spi_message *spi_msg)
{
	...
	en_cfg.param = 0;					// src no loop:important
	sddev_control(DD_DEV_TYPE_GDMA, CMD_GDMA_CFG_DSTADDR_LOOP, &en_cfg);
...
void spi_dma_tx_enable(UINT8 enable)
{
	...
	sddev_control(DD_DEV_TYPE_GDMA, CMD_GDMA_SET_DMA_ENABLE, &en_cfg);
...
void bk_spi_slave_dma_config(UINT32 mode, UINT32 rate)
{
	...
	//disable rx/tx over
	param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_RXOVR_EN, (void *)&param);

	param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_TXOVR_EN, (void *)&param);

	//disable CSN intterrupt
	param = 0;
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_CS_EN, (void *)&param);
...
int bk_spi_slave_dma_rx_init(UINT32 mode, UINT32 rate, struct spi_message *spi_msg)
{
	...
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_RXTRANS_LEN, (void *)&recv_len);
...
int bk_spi_slave_dma_tx_init(UINT32 mode, UINT32 rate, struct spi_message *spi_msg)
{
	...
	sddev_control(DD_DEV_TYPE_SPI, CMD_SPI_TXTRANS_LEN, (void *)&send_len);
...
int bk_spi_slave_dma_transfer(struct spi_message *spi_msg)
{
	...
	{
#if BK_SPI_DEBUG
		spi_debug_prt(SPI_TX_DMA_CHANNEL);
#endif
	...
	{
#if BK_SPI_DEBUG
		spi_debug_prt(SPI_RX_DMA_CHANNEL);
#endif
