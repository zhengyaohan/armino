...
static void bk_spi_slave_configure(UINT32 rate, UINT32 mode)
{
	...
	/* MSB first */
	param = 0;
	sddev_control(DD_DEV_TYPE_SPI_SLECT, CMD_SPI_LSB_EN, (void *)&param);
...
int bk_spi_slave_xfer(struct spi_message *msg)
{
    ...
    BK_SPI_PRT("send_len:%d,recv_len:%d\r\n",send_len,recv_len);

    /*tx len*/
    if(send_len > 0)
    {
        param = send_len - 1;//new spi hardware bug
    }
    else
    {
        param = 0;
    ...
    /*rx fifo int level*/
    param = 0;
    sddev_control(DD_DEV_TYPE_SPI_SLECT, CMD_SPI_RXINT_MODE,(void *)&param);
	...
	if ((send_ptr) && send_len) {
		...
		/* RX_EN enable */
		param = 1;
		sddev_control(DD_DEV_TYPE_SPI_SLECT, CMD_SPI_RX_EN, (void *)&param);
