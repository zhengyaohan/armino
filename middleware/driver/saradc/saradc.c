...
{
	bk_int_isr_register(INT_SRC_SARADC, saradc_isr, NULL);

	ddev_register_dev(DD_DEV_TYPE_SARADC, (DD_OPERATIONS *)&saradc_op);
...
{
	ddev_unregister_dev(DD_DEV_TYPE_SARADC);
}
...
#if ((CONFIG_SOC_BK7231U) || (CONFIG_SOC_BK7251) || (CONFIG_SOC_BK7231N) || (CONFIG_SOC_BK7256XX))
	case 4: {
...
#endif  // ((CONFIG_SOC_BK7231U) || (CONFIG_SOC_BK7251))
...
void saradc_calculate_step1(void)
{
	...
	sys_drv_set_gadc_ten(1);// to do,need remove old interface after all adaption is finished
	#else
	int reg;
...
#else
    int reg;
    reg = sddev_control(DD_DEV_TYPE_SCTRL, CMD_SCTRL_GET_ANALOG4, &reg);
	reg |= (1 << 1);
	reg = sddev_control(DD_DEV_TYPE_SCTRL, CMD_SCTRL_SET_ANALOG4, &reg);
#endif
...
#if (!CONFIG_SOC_BK7271)
	#if (CONFIG_SOC_BK7256XX) || (CONFIG_SOC_BK7256_CP1)
	sys_drv_set_gadc_ten(0);// to do,need remove old interface after all adaption is finished
	#else
	int reg;
	reg = sddev_control(DD_DEV_TYPE_SCTRL, CMD_SCTRL_GET_ANALOG7, &reg);
	reg &= (~(1 << 21));
