...
#define MB_PHY_CMD_CHNL		(MAILBOX_BOX0 + MB_PHY_ASYNC_TX)
#define MB_PHY_ACK_CHNL		(MAILBOX_BOX1 + MB_PHY_ASYNC_TX)
...
static u8 mb_phy_chnl_tx_cmd(u8 log_chnl)
{
	...
	cmd_ptr->hdr.tx_seq = phy_chnl_cb.tx_seq & 0x7F;
	cmd_ptr->hdr.rsp_flag = 0;
	cmd_ptr->hdr.logical_chnl = log_chnl;

	chnl_type += cmd_ptr->hdr.rsp_flag; 	/* MB_PHY_CMD_CHNL */
	...
	{
		log_chnl_cb[log_chnl].tx_isr(log_chnl);  	/* phy_chnl is BUSY now, tx_isr will not trigger phy_chnl_start_tx. */
	}
...
static void mb_phy_chnl_rx_ack_isr(mb_phy_chnl_ack_t *ack_ptr)
{
	...
	if( (ack_ptr->hdr.logical_chnl != phy_chnl_cb.tx_log_chnl_id) || 
		(ack_ptr->hdr.tx_seq != (phy_chnl_cb.tx_seq & 0x7F)) )
	{
	...
	if(log_chnl_cb[phy_chnl_cb.tx_log_chnl_id].tx_cmpl_isr != NULL)
	{
		/* clear following header members. */
		ack_ptr->hdr.logical_chnl = 0;
		ack_ptr->hdr.rsp_flag     = 0;
		ack_ptr->hdr.tx_seq       = 0;

		/* hdr.rx_fail, hdr.cmd these 2 members keep untouched. */

		log_chnl_cb[phy_chnl_cb.tx_log_chnl_id].tx_cmpl_isr(phy_chnl_cb.tx_log_chnl_id, (mb_chnl_ack_t *)ack_ptr);
	}
...
static void mb_phy_chnl_rx_cmd_isr(mb_phy_chnl_cmd_t *cmd_ptr)
{
	...
	{
		cmd_ptr->hdr.data = cmd_ptr->hdr.cmd;   /* clear all other hdr members except hdr.cmd. */
		log_chnl_cb[log_chnl].rx_isr(log_chnl, (mb_chnl_cmd_t *)cmd_ptr);

		/* !!!! cmd_ptr buffer now contains ACK data !!!!. */
	}
	else
	{
		cmd_ptr->hdr.rx_fail = 1;     		/* cmd NO target app, it is an ACK bit to peer CPU. */
		hdr_data = cmd_ptr->hdr.data;		/* update the rx_fail flag to hdr_data. */
	}

	/* mb_phy_chnl_tx_ack. */

	/* RE-USE the cmd buffer for ACK !!! */
	cmd_ptr->hdr.data= hdr_data;
	cmd_ptr->hdr.rsp_flag = 1;			/*  rsp_flag MUST be 1, use the ACK channel.  */

	chnl_type += cmd_ptr->hdr.rsp_flag;	/* MB_PHY_ACK_CHNL */
...
static void mb_phy_chnl_rx_isr(mailbox_data_t * mb_data)
{
	 ...
	 * when rsp_flag is 1, it means from ack box ( MAILBOXn_BOX1 ).
	 */
	if(rx_data->phy_ch_hdr.rsp_flag)		/* rx ack. */
	{
...
bk_err_t mb_chnl_init(void)
{
	...
	for(int i = 0; i < MB_LOG_CHNL_NUM; i++)
	{
