...
void sys_hal_enter_deep_sleep(void * param)
{
	...
	clock_value &= ~(1 << 11);//en_audpll
	clock_value &= ~(1 << 9);//en_psram_ldo
	clock_value &= ~(1 << 8);//en_dco
	clock_value &= ~(1 << 7);//en_xtall
	clock_value &= ~(1 << 6);//en_usb
    sys_ll_set_ana_reg6_value(clock_value);

	clock_value = 0;
    clock_value = sys_ll_get_ana_reg5_value();
	clock_value &= ~(1 << 13);//global central bias enable
...
void sys_hal_usb_wakeup_enable(uint8_t index, uint8_t type)
{
...
void sys_hal_analog_set(analog_reg_t reg, uint32_t value)
{
	...
	REG_WRITE(analog_reg_address, value);
}
uint32_t sys_hal_analog_get(analog_reg_t reg)
{
	...
	return REG_READ(analog_reg_address);
}
...
void sys_hal_cpu0_i2s_int_en(uint32_t value)
{
