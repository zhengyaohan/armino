...
int32 sys_hal_set_int_status(uint32 param) // CMD_CLR_INTR_STATUS
{
	uint32 reg = 0;
	reg = REG_READ(ICU_INT_STATUS);
	REG_WRITE(ICU_INT_STATUS, (reg | param));
	return 0;
}
...
uint32 sys_hal_set_fiq_reg_status(uint32 param)
{
	...
	uint32 reg = 0;
	reg = REG_READ(ICU_FIQ_STATUS);
	REG_WRITE(ICU_FIQ_STATUS, (reg | param));
#endif
...
uint32 sys_hal_set_intr_raw_status(uint32 param) //CMD_CLR_INTR_RAW_STATUS
{
	...
	reg = REG_READ(ICU_INT_RAW_STATUS);
	REG_WRITE(ICU_INT_RAW_STATUS, (reg | param));
...
void sys_hal_interrupt_status_set(uint32_t value)
{
    ...
    reg = REG_READ(ICU_INT_STATUS);
    REG_WRITE(ICU_INT_STATUS, (reg | value));
}
