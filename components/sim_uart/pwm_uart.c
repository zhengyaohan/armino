...
#if(CONFIG_FPGA)  // FPGA:PWM0-2-32kCLK, pwm3-5-24CLK
...
static void UARTCALL init_pwm_param_2(pwm_param_t *pwm_param)
{
	...
	*((volatile UINT32 *)(ICU_R_INT_EN)) = *((volatile UINT32 *)(ICU_R_INT_EN)) | (IRQ_PWM_BIT);
}
...
void uart_io_sim_disable(void)
{
	...
	ret = sddev_control(DD_DEV_TYPE_PWM, CMD_PWM_UNIT_DISABLE, &param);
	BK_ASSERT(PWM_SUCCESS == ret);
