#!/bin/bash -e
#
# This script assumes that Python3 and pip3 are already installed on the host OS
#
...
install_pip_package() {
    ...
    if ! pip3 show "$PACKAGE_TO_INSTALL" > /dev/null 2>&1; then
        ...
        case "$(uname)" in
            "Darwin")
                pip3 install --user "$PACKAGE_TO_INSTALL"=="$PACKAGE_VERSION_TO_INSTALL"
                ;;
            "Linux")
                pip3 install --user "$PACKAGE_TO_INSTALL"=="$PACKAGE_VERSION_TO_INSTALL"
                ;;
...
if ! python3 --version > /dev/null 2>&1; then
    echo "*****************************************************************************"
    echo "To generate documentation for HomeKit ADK please install Python3 for your OS."
    echo "*****************************************************************************"
    exit 1
fi
...
install_doxygen
install_pip
install_pip_package docutils 0.16
install_pip_package sphinx 2.4.4
install_pip_package sphinx_rtd_theme 0.5.1
install_pip_package breathe 4.14.1
install_pip_package exhale 0.2.3
install_pip_package recommonmark 0.7.1
install_pip_package sphinx-markdown-tables 0.0.15
install_pip_package sphinx-tabs 3.1.0

echo "*****************"
echo "Running sphinx..."
echo "*****************"
"$(python3 -m site --user-base)/bin/sphinx-build" -M html "$DOC_SRC_DIR" "$DOC_OUT_DIR"
