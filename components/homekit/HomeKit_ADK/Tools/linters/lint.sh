...
finish() {
    ...
    if ${CI:-false} && [[ $rc -eq 1 ]]; then
        echo "**********************************************************************"
        echo "Code doesn't comply with style rules and can be auto-fixed by running:"
        echo ""
        echo "$0 -c"
        echo "**********************************************************************"
    fi
...
usage()
{
    ...
    echo "-s  - Skip code convention check."
    echo ""
    echo "EXAMPLES: "
...
FILE_ARGS=()
VERBOSE=false
SKIP_CODE_CONVENTION=false

while getopts "hcvsf:d:" opt; do
    case ${opt} in
        f ) FILE_ARGS+=( "$OPTARG" );;
        d ) SEARCH_PATH="$OPTARG";;
        c ) CORRECT=true;;
        s ) SKIP_CODE_CONVENTION=true;;
        v ) VERBOSE=true;;
        h ) usage;;
        \? ) usage;;
    esac
...
if [ ${#FILE_ARGS[@]} -gt 0 ]; then
    ...
    if [[ ${#SOURCE_FILES[@]} -eq 0 && ${#SHELL_FILES[@]} -eq 0 && ${#PYTHON_FILES[@]} -eq 0 ]]; then
...
# 2. Do source code formatting (if there are source files to check).
if [ ${#SOURCE_FILES[@]} -gt 0 ] || [ ${#FILE_ARGS[@]} -eq 0 ]; then
    "$ADK_ROOT"/Tools/linters/clint.sh "${ARG_SEARCH_PATH[@]}" "${ARG_CORRECT}" "${SOURCE_FILES[@]/#/-f}"
fi

# 3. Do source code convention (if there are source files to check).
if [ ${#SOURCE_FILES[@]} -gt 0 ] || [ ${#FILE_ARGS[@]} -eq 0 ]; then
    if [ "$SKIP_CODE_CONVENTION" = "false" ]; then
        "$ADK_ROOT"/Tools/linters/check_code_convention.sh "${ARG_SEARCH_PATH[@]}" "${SOURCE_FILES[@]/#/-f}"
    fi
fi

# 4. Do python lint (if there are python files to check).
if [ ${#PYTHON_FILES[@]} -gt 0 ] || [ ${#FILE_ARGS[@]} -eq 0 ]; then
    if [ "$SKIP_CODE_CONVENTION" = "false" ]; then
        "$ADK_ROOT"/Tools/linters/pylint.sh "${ARG_SEARCH_PATH[@]}" "${ARG_CORRECT}" "${PYTHON_FILES[@]/#/-f}"
    fi
fi
