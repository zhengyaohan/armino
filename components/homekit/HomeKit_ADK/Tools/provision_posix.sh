...
while (( $# )); do
    case "${1}" in
        ...
        "--wac")
            shift
            if (( supportsWAC )); then
                fail "--wac specified multiple times."
            fi
            supportsWAC=1
            ;;
...
if [ "${fixedProductData}" == "" ]; then
    fail "product data not specified."
fi
...
if (( ! supportsDisplay )); then
    echo "================================================================================"
    echo " NOTE: The following setup payload is only valid if accurate Setup Code"
