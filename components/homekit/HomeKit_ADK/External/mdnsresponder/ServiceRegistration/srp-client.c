...
#define SRP_BASE_REATTEMPT_TIME_MS (1000 * 20)
    ...
    if (context->next_retransmission_time == 0) {
        ...
        for (registration = client->registrations; registration; registration = registration->next) {
            if (registration->callback != NULL &&
                registration->serial <= context->serial &&
...
do_srp_update(client_state_t *client, bool definite, bool *did_something)
{
    ...
    if (active_update == NULL) {
        ...
        // If this update times out, try again later, backing off to an hour exponentially.
        active_update->next_attempt_time = SRP_BASE_REATTEMPT_TIME_MS;
...
srp_deregister(void *os_context)
{
    ...
    if (something_to_deregister) {
        ...
        client->active_update->next_attempt_time = SRP_BASE_REATTEMPT_TIME_MS;
