...
HAP_DIAGNOSTIC_PUSH
HAP_DIAGNOSTIC_IGNORED_CLANG("-Wgnu-zero-variadic-macro-arguments")
...
#define DEBUG(fmt, ...)
#endif // DEBUG VERBOSE
#endif // LOG_SYSLOG_STDERR
#endif // LOG_FPRINTF_STDERR
...
#define IS_THREAD_SRP_SERVICE(service) \
...
#define IS_THREAD_PREF_ID_SERVICE(service) \
    ((cti_service)->enterprise_number == THREAD_ENTERPRISE_NUMBER &&    \
...
#ifndef OPEN_SOURCE
#  ifdef THREAD_SERVICE_SEND_BOTH
#    define APPLE_ENTERPRISE_NUMBER  ((uint64_t)52627)
#    define APPLE_SRP_SERVER_OPTION  1
#    define APPLE_PREF_ID_OPTION     2
#    define IS_APPLE_SRP_SERVICE(service) \
        ((cti_service)->enterprise_number == APPLE_ENTERPRISE_NUMBER && \
         (cti_service)->service_type == APPLE_SRP_SERVER_OPTION &&      \
         (cti_service)->service_version == 1 &&                         \
         (cti_service)->server_length == 18)
#    define IS_APPLE_PREF_ID_SERVICE(service) \
        ((cti_service)->enterprise_number == APPLE_ENTERPRISE_NUMBER && \
         (cti_service)->service_type == APPLE_PREF_ID_OPTION &&         \
         (cti_service)->service_version == 1 &&                         \
         (cti_service)->server_length == 9)
#    define IS_SRP_SERVICE(service) (IS_APPLE_SRP_SERVICE(service) || IS_THREAD_SRP_SERVICE(service))
#    define IS_PREF_ID_SERVICE(service) (IS_APPLE_PREF_ID_SERVICE(service) || IS_THREAD_PREF_ID_SERVICE(service))
#  else
#    define IS_SRP_SERVICE(service) IS_THREAD_SRP_SERVICE(service)
#    define IS_PREF_ID_SERVICE(service) IS_THREAD_PREF_ID_SERVICE(service)
#  endif // THREAD_SERVICE_SEND_BOTH
#else
#    define IS_SRP_SERVICE(service) IS_THREAD_SRP_SERVICE(service)
#    define IS_PREF_ID_SERVICE(service) IS_THREAD_PREF_ID_SERVICE(service)
#endif // OPEN_SOURCE
