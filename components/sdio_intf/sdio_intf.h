...
} __hpacked SCANU_RET_IND_S, *SCANU_RET_IND_PTR;
...
} __packed IEEE80211_PROBE_RSP_S, *IEEE80211_PROBE_RSP_PTR;
...
#define SCMD_802_11_DISCONNECT			0x0052
...
typedef struct mrvl_ie_header {
	UINT16 type;
	UINT16 len;
} __packed MRVL_IE_HDR_S, *MRVL_IE_HDR_PTR;


typedef struct mrvl_ie_ssid_param_set {
	struct mrvl_ie_header header;
	UINT8 ssid[1];
} __packed MRVL_IE_SSID_PARAM_S, *MRVL_IE_SSID_PARAM_PTR;

typedef struct chanscanmode {
	UINT8 passivescan: 1;
	UINT8 disablechanfilt: 1;
	UINT8 reserved_2_7: 6;
} __packed CHAN_SCAN_MODE_S, *CHAN_SCAN_MODE_PTR;
...
typedef struct chanscanparamset {
	...
	UINT16 minscantime;
	UINT16 maxscantime;
} __packed CHAN_SCAN_PARAM_S, *CHAN_SCAN_PARAM_PTR;
...
typedef struct stm32_cmd_hdr {
	...
	UINT16 seqnum;
	UINT16 result;
} __packed STM32_CMD_HDR_S, *STM32_CMD_HDR_PTR;
...
typedef struct stm32_txpd {
	...
	/* reserved */
	UINT8 reserved1;
} __packed STM32_TXPD_S, *STM32_TXPD_PTR;
...
typedef struct stm32_rxpd {
	...
	UINT8 priority;
	UINT8 reserved[3];
} __packed STM32_RXPD_S, *STM32_RXPD_PTR;
...
typedef struct beacon_info {
	...
	UINT16 capability;
} __packed BEACON_INFO_S, *BEACON_INFO_PTR;
...
typedef struct cmd_ds_802_11_scan_rsp {
	...
	UINT8 tlvbuffer[1];
} __packed STM32_SCAN_RSP_S, *STM32_SCAN_RSP_PTR;
...
typedef struct _stm32_tx_frame_ {
	...
	void *private;
} __packed STM32_TX_FRAME;

// c.f. struct txdesc_api
struct stm32_data_tx_frame {
	struct _stm32_frame_hdr hdr;
...
typedef struct stm32_cmd_802_11_scan {
	...
	UINT8 bssid[ETH_ALEN];
	UINT8 tlvbuffer[1];
} __packed STM32_CMD_802_11_SCAN_S, *STM32_CMD_802_11_SCAN_PTR;
...
} __packed STM32_KEY_MATERIAL_S, *STM32_KEY_MATERIAL_PTR;
...
typedef struct cmd_ds_802_11_associate_response {
	...
	UINT16 aid;
	UINT8 iebuf[512];
} __packed ASSOC_RESP_S, *ASSOC_RESP_PTR;
...
} __packed ASSOC_REQ_S, *ASSOC_REQ_PTR;

typedef struct cmd_ds_802_11_disconnect {
	STM32_CMD_HDR_S hdr;
	UINT16 reason_code;
	UINT8  vif_index;
} __packed DISCONNECT_REQ_S, *DISCONNECT_REQ_PTR;

typedef struct cmd_ds_802_11_disconnect_response {
	STM32_CMD_HDR_S hdr;
	...
	bool ft_over_ds;
} __packed DISCONNECT_RESP_S, *DISCONNECT_RESP_PTR;
