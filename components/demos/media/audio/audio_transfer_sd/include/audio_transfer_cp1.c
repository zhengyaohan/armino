...
#define  AUD_DEBUG  0
#define  AUDIO_TRANSFER_AEC_SOFTWARE_MODE  1
...
//beken_semaphore_t audio_mailbox_sem = NULL;
...
#endif
bk_gpio_set_output_high(GPIO_15);
...
bk_gpio_set_output_low(GPIO_15);
	return ret;
}
...
static bk_err_t audio_write_req_process(void)
{
	...
	bk_gpio_set_output_high(GPIO_18);
	...
	bk_gpio_set_output_low(GPIO_18);
...
/*	ret = rtos_init_semaphore(&audio_mailbox_sem, 1);
	if (ret) {
		os_printf("create audio mailbox semaphore fail.\r\n");
		return;
	}
*/

	bk_gpio_enable_output(GPIO_15);
	bk_gpio_enable_output(GPIO_14);
	bk_gpio_enable_output(GPIO_18);
