...
static bk_err_t audio_cp1_send_aud_mailbox_msg(audio_mailbox_msg_t *msg)
{
	bk_err_t ret = BK_OK;
	...
	audio_cp1_mailbox_busy_status = true;
...
static void audio_cp1_mailbox_rx_isr(aud_mb_t *aud_mb, mb_chnl_cmd_t *cmd_buf)
{
	...
	switch(cmd_buf->hdr.cmd) {
		...
		case AUD_MB_GET_ENCODER_USED_SIZE_REQ_CMD:
			...
			audio_cp1_send_aud_mailbox_msg(&mb_msg);
			return;
...
/*	size = ring_buffer_get_fill_size(&mic_rb);
	...
	size = ring_buffer_get_fill_size(&ref_rb);
...
static bk_err_t audio_encoder_process(void)
{
	...
	if (ring_buffer_get_free_size(&encoder_rb) >= frame_sample) {
...
/*
		size = ring_buffer_get_fill_size(&speaker_rb);
		if (size == PSRAM_AUD_DECD_RING_BUFF_SIZE*2) {
			// the speaker ring buffer is full and tell cpu1 to start audio transfer
			msg.op = AUDIO_CP1_START;
			ret = audio_send_msg(msg);
			if (ret != kNoErr) {
				os_printf("cp1: send msg: %d fails \r\n", msg.op);
			...
			os_printf("speaker ring buff full and start transfer \r\n");
		}
*/
		...
		//os_printf("cp1: send mailbox: AUD_MB_WRITE_DAC_DATA_REQ_CMD \r\n");
	}
...
static bk_err_t audio_read_req_process(void)
{
	...
	mb_msg.mb_cmd = AUD_MB_READ_ADC_DATA_DONE_CMD;
...
static bk_err_t audio_write_req_process(void)
{
	...
	if (ring_buffer_get_free_size(&decoder_rb) >= write_buffer_size) {
		size = ring_buffer_write(&decoder_rb, (uint8_t*)write_buffer, write_buffer_size);
		if (size != write_buffer_size) {
			os_printf("cp1: the data size written to decoder_ring_buff is not write_buffer_size:%x \r\n", write_buffer_size);
			//return BK_FAIL;
		}
	}
#if AUD_TRANS_DEBUG_DAC
	...
	ret = audio_cp1_send_aud_mailbox_msg(&mb_msg);
	if (ret != BK_OK) {
...
static void audio_start_transfer_process(void)
{
	...
	size = ring_buffer_write(&speaker_rb, (uint8_t *)pcm_data, AUD_DECD_FRAME_SAMP_SIZE*2);
	if (size != AUD_DECD_FRAME_SAMP_SIZE*2) {
		os_printf("cp1: the data writeten to speaker_ring_buff error, size=%d \r\n", size);
		return;
	}
...
/*
	size = ring_buffer_write(&ref_rb, (uint8_t *)pcm_data, AUD_DECD_FRAME_SAMP_SIZE*2);
	if (size != AUD_DECD_FRAME_SAMP_SIZE*2) {
		os_printf("cp1: the data writeten to ref_ring_buff error, size=%d \r\n", size);
		return;
	}
*/
...
*/
	...
	/* notify cpu0 to read encoder data by mailbox msg, and ignore the ack */
	mb_msg.mb_cmd = AUD_MB_START_TRANSFER_DONE_CMD;
	mb_msg.param1 = 0;
	mb_msg.param2 = 0;
	mb_msg.param3 = 0;
	//ret = aud_mailbox_send_msg(&msg);
	ret = audio_cp1_send_aud_mailbox_msg(&mb_msg);
	if (ret != BK_OK) {
		os_printf("send mailbox msg: AUD_MB_START_TRANSFER_DONE_CMD fail \r\n");
	}
...
static bk_err_t audio_stop_transfer(void)
{
	...
	mb_msg.param2 = 0;
	mb_msg.param3 = 0;
	//return aud_mailbox_send_msg(&mb_msg);
	return audio_cp1_send_aud_mailbox_msg(&mb_msg);
}
