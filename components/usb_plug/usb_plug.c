...
#if CONFIG_USB && (CONFIG_SOC_BK7251)
...
int wifi_get_charge_cal_from_efuse(UINT8 *cal)
{
	...
	for (i = 0; i < EFUSE_CHARGE_CAL_LEN; i++) {
		efuse_addr = EFUSE_CHARGE_CAL_ADDR + i;
		efuse_data = 0;
...
{
	UINT32 charge_type = 0;
	sys_drv_usb_charge_ctrl(charge_type, NULL);
}
...
#endif
	sys_drv_usb_charge_ctrl(charge_type, (void *)&charge_cfg);
...
int usb_charge_set_cal_default_reg(void)
{
	...
	REG_WRITE(SCTRL_ANALOG_CTRL3, reg);
...
int vbat_adc_cal_step1(void)
{
	...
	reg = REG_READ(SCTRL_ANALOG_CTRL2);
	reg &= ~(1 << 22);
	REG_WRITE(SCTRL_ANALOG_CTRL2, reg);
	...
	reg = REG_READ(SCTRL_ANALOG_CTRL7);
	reg &= ~(1 << 30);
	REG_WRITE(SCTRL_ANALOG_CTRL7, reg);
/*
	reg = sddev_control(DD_DEV_TYPE_SCTRL, CMD_SCTRL_GET_ANALOG2, &reg);
	reg &= ~(1 << 22);
	reg = sddev_control(DD_DEV_TYPE_SCTRL, CMD_SCTRL_SET_ANALOG2, &reg);
*/

/*
	reg = sddev_control(DD_DEV_TYPE_SCTRL, CMD_SCTRL_GET_ANALOG7, &reg);
	reg &= ~(1 << 30);
	reg = sddev_control(DD_DEV_TYPE_SCTRL, CMD_SCTRL_SET_ANALOG7, &reg);
*/
	// 2
	reg = REG_READ(SCTRL_ANALOG_CTRL7);
	reg &= ~(1 << 20);
...
void usb_charger_calibration(void)
{
	...
	sys_drv_usb_charge_cal(CHARGE_STEP1, &chrg);
	sys_drv_usb_charge_cal(CHARGE_STEP3, &chrg);
	sys_drv_usb_charge_cal(CHARGE_STEP4, &chrg);
...
void usb_plug_func_open(void)
{
	...
	usb_plug_hdl = ddev_open(DD_DEV_TYPE_USB_PLUG, &status, (UINT32)&user_plug);
