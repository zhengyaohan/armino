...
static void uvc_intf_frame_end_handler(void)
{
#if 0
#if CONFIG_GENERAL_DMA
...
{
	os_memset(&uvc_intf, 0, sizeof(UVC_DESC_ST));
...
bk_err_t uvc_register_frame_end_isr(void *callback)
{
	...
	uvc_intf.end_frame_handler = callback;
