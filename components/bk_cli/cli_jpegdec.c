...
{
	CLI_LOGI("lcd_jpegdec {init|display_picture|stop_display|uvc_dispaly_init|start_uvc|close_uvc|deinit}\n");
}
...
void lcd_jpeg_dec_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "init") == 0) {
		...
		if (argc != 5) {
			CLI_LOGE("init: width height ratio\r\n");
			return;
		}
		...
		if (err != kNoErr) {
			CLI_LOGE("init jpeg_decoder failed\r\n");
			return;
		}
		...
		if (argc != 4) {
			CLI_LOGE("picture_jpegdec_to_sdcard: picture_name output_file_name\r\n");
			return;
		}
		...
		if (fr != FR_OK) {
			CLI_LOGE("read file fail.\r\n");
			return;
		}

		fr = f_close(&file);
		if (fr != FR_OK) {
			CLI_LOGE("close %s fail!\r\n", filename);
			return;
		}
		...
		if (argc != 3) {
			CLI_LOGE("display_picture: picture_name\r\n");
			return;
		}
		...
		if (argc != 4) {
			CLI_LOGE("uvc_dispaly_init: pps fps\r\n");
			return;
		}
		...
		if (err != kNoErr) {
			CLI_LOGE("uvc set ppi and fps failed!\r\n");
			return;
		}
