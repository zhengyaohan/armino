...
#define IPC_RSP_TIMEOUT			50		/* 50ms */
...
static u32 ipc_server_cmd_handler(ipc_chnl_cb_t *chnl_cb, mb_chnl_ack_t *ack_buf)
{
	...
	ipc_rsp->rsp_buff = chnl_cb->rx_xchg_buff; //ipc_xchg_buf[chnl_cb->chnl_id].client_buff;
	...
	switch(chnl_cb->rx_cmd)
	{
		...
		case IPC_RES_ACQUIRE_CNT:
			if(chnl_cb->cmd_len >= sizeof(amp_res_req_t))
			{
				amp_res_req_t * res_req = (amp_res_req_t *)chnl_cb->cmd_buf;
