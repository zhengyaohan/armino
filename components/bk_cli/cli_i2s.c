...
{
	CLI_LOGI("i2s_master_test {start|stop} \r\n");
	CLI_LOGI("i2s_master_sin_test {start|stop} \r\n");
	CLI_LOGI("i2s_master_mic_test {start|stop} \r\n");
	CLI_LOGI("i2s_slave_test {start|stop} \r\n");
	CLI_LOGI("i2s_slave_mic_test {start|stop} \r\n");
}
...
{
	//CLI_LOGI("enter cli_i2s_txint_isr \r\n");
}
...
{
	//CLI_LOGI("enter cli_i2s_slave_rxovf_isr \r\n");
	//change test_status: testing-->stop
	if (i2s_test_global.test_status == I2S_TESTING) {
		//CLI_LOGI("enter cli_i2s_slave_rxovf_isr \r\n");
		i2s_test_global.test_status = I2S_STOP;
...
static void test_log_print(i2s_rate_table_node_t rate_node)
{
	switch (rate_node.samp_rate)
	{
		case I2S_SAMP_RATE_8000:
			CLI_LOGI("========= CASE sample_rate: 8K, data_lenght: %d ============\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_12000:
			CLI_LOGI("========= CASE sample_rate: 12K, data_lenght: %d ============\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_16000:
			CLI_LOGI("========= CASE sample_rate: 16K, data_lenght: %d ============\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_24000:
			CLI_LOGI("========= CASE sample_rate: 24K, data_lenght: %d ============\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_32000:
			CLI_LOGI("========= CASE sample_rate: 32K, data_lenght: %d =============\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_48000:
			CLI_LOGI("========= CASE sample_rate: 48K, data_lenght: %d =============\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_96000:
			CLI_LOGI("========= CASE sample_rate: 96K, data_lenght: %d ==============\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_8018:
			CLI_LOGI("========= CASE sample_rate: 8.0182K, data_lenght: %d ==========\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_11025:
			CLI_LOGI("========= CASE sample_rate: 11.025K, data_lenght: %d ===========\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_22050:
			CLI_LOGI("========= CASE sample_rate: 22.050K, data_lenght: %d ============\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_44100:
			CLI_LOGI("========= CASE sample_rate: 44.1K, data_lenght: %d ==============\r\n", rate_node.datawidth);
			break;

		case I2S_SAMP_RATE_88200:
			CLI_LOGI("========= CASE sample_rate: 88.2K, data_lenght: %d ===============\r\n", rate_node.datawidth);
			break;
...
static void i2s_master_test(void)
{
	...
	for (i = 0; i < sizeof(i2s_rate_table)/sizeof(i2s_rate_table_node_t); i++)
	{
		...
		CLI_LOGI("enable i2s rxovf interrupt successful \r\n");
		...
		if (ret != BK_OK) {
			CLI_LOGI("config i2s sample_rate and data_length fail \r\n");
			CLI_LOGI("=================================== CASE TEST FAIL ==================================\r\n");
			continue;
		}
		CLI_LOGI("config i2s sample_rate and data_length \r\n");
...
//			i2s_test_global.store_mode = i2s_config.store_mode;
		...
		if (read_flag) {
			...
			CLI_LOGI("i2s master receive i2s data from slave: %08x \r\n", data_buf);
			bk_i2s_clear_rxfifo();
		}
		receive_data_check += 1;
		if (data_buf == receive_data_check) {
			CLI_LOGI("i2s master receiving and slave sending test result: PASS \r\n");
			CLI_LOGI("=================================== CASE TEST PASS ==================================\r\n");
		} else {
			CLI_LOGI("i2s master receiving and slave sending test result: FAIL \r\n");
			CLI_LOGI("=================================== CASE TEST FAIL ==================================\r\n");
		}

		//disable i2s
		bk_i2s_enable(I2S_DISABLE);
		CLI_LOGI("disable i2s \r\n");

		//clear rxfifo and txfifo
		bk_i2s_clear_rxfifo();
		bk_i2s_clear_txfifo();
		CLI_LOGI("clear rxfifo and txfifo \r\n");
...
static void cli_i2s_master_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "start") == 0) {
		CLI_LOGI("i2s master test start \r\n");
		...
		CLI_LOGI("########################### I2S MODE: Philips TEST START ##########################\r\n");
		bk_i2s_set_work_mode(I2S_WORK_MODE_I2S);
		i2s_master_test();
		CLI_LOGI("########################### I2S MODE: Philips TEST END ############################\r\n");
		CLI_LOGI("--------------------------------------------------------------------------------------------\r\n");

		CLI_LOGI("########################### I2S MODE: Left Justified TEST START ###################\r\n");
		bk_i2s_set_work_mode(I2S_WORK_MODE_LEFTJUST);
		i2s_master_test();
		CLI_LOGI("########################### I2S MODE: Left Justified TEST END #####################\r\n");
		CLI_LOGI("--------------------------------------------------------------------------------------------\r\n");

		CLI_LOGI("########################### I2S MODE: Right Justified TEST START ###################\r\n");
		bk_i2s_set_work_mode(I2S_WORK_MODE_RIGHTJUST);
		i2s_master_test();
		CLI_LOGI("########################### I2S MODE: Right Justified TEST END #####################\r\n");
		CLI_LOGI("--------------------------------------------------------------------------------------------\r\n");

		CLI_LOGI("i2s master test complete \r\n");
	} else if (os_strcmp(argv[1], "stop") == 0) {
		CLI_LOGI("i2s master test stop \r\n");
		bk_i2s_driver_deinit();
		CLI_LOGI("i2s master test stop successful \r\n");
	} else {
		cli_i2s_help();
		return;
	}
...
static void cli_i2s_master_sin_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	i2s_config_t i2s_config;
	i2s_rate_t rate;
	...
	if (os_strcmp(argv[1], "start") == 0) {
		...
		while (1)
		{
			for (i = 0; i<I2S_TEST_DATA_SIZE; i++)
			{
				...
				//delay(10);
				//CLI_LOGI("write data: 0x%08x\r\n", data_source[i]);
			}
		}

		CLI_LOGI("start i2s master send test successful\r\n");
	} else if (os_strcmp(argv[1], "stop") == 0) {
		CLI_LOGI("i2s sin test stop\n");
		bk_i2s_driver_deinit();
		CLI_LOGI("i2s sin test stop successful\n");
	} else {
		cli_i2s_help();
		return;
	}
...
static void cli_i2s_master_mic_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "start") == 0) {
		...
		CLI_LOGI("init audio driver, adc successful\n");
...
//			i2s_test_global.store_mode = i2s_config.store_mode;
		...
		receive_data_check -= 1;
		if (data_buf == receive_data_check) {
			CLI_LOGI("i2s master sending and slave receiving test result: PASS \r\n");
			CLI_LOGI("=================================== CASE TEST PASS ==================================\r\n");
		} else {
			CLI_LOGI("i2s master sending and slave receiving test result: FAIL \r\n");
			CLI_LOGI("=================================== CASE TEST FAIL ==================================\r\n");
		}
...
static void cli_i2s_slave_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	i2s_config_t i2s_config;
	...
	if (os_strcmp(argv[1], "start") == 0) {
		...
		CLI_LOGI("i2s slave test complete\r\n");
	} else if (os_strcmp(argv[1], "stop") == 0) {
		CLI_LOGI("i2s slave test stop \r\n");
		bk_i2s_driver_deinit();
		CLI_LOGI("i2s slave test stop successful \r\n");
	} else {
		cli_i2s_help();
		return;
	}
...
static void cli_i2s_slave_mic_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "start") == 0) {
		...
		CLI_LOGI("init audio driver, dac successful\n");
		...
		CLI_LOGI("start i2s slave receive test successful\r\n");
	} else if (os_strcmp(argv[1], "stop") == 0) {
		CLI_LOGI("audio adc mcp test stop\n");
		bk_i2s_driver_deinit();
		CLI_LOGI("audio adc mcp test stop successful\n");
