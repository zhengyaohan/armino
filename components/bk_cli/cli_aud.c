...
#if 0
static void cli_aud_file_to_dac_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	bk_err_t ret = BK_OK;
	aud_dac_config_t dac_config;
	dma_config_t dma_config;
	...
	uint32_t *data_buffer = os_malloc(test_size);
	UINT uiTemp = 0;
	...
	if (os_strcmp(argv[1], "start") == 0) {
		...
		eq_config.flt0_A1 = ~(-2075115);
		eq_config.flt0_A2 = ~(1026817);
		eq_config.flt0_B0 = 1046487;
		eq_config.flt0_B1 = -2075115;
		eq_config.flt0_B2 = 1028905;

		eq_config.flt1_A1 = ~(-2068839);
		eq_config.flt1_A2 = ~(1021371);
		eq_config.flt1_B0 = 1051295;
		eq_config.flt1_B1 = -2068839;
		eq_config.flt1_B2 = 1018652;

		eq_config.flt2_A1 = ~(-1671107);
		eq_config.flt2_A2 = ~(630617);
		eq_config.flt2_B0 = 948125;
		eq_config.flt2_B1 = -1671107;
		eq_config.flt2_B2 = 731068;

		eq_config.flt3_A1 = ~(313631);
		eq_config.flt3_A2 = ~(-421313);
		eq_config.flt3_B0 = 1051116;
		eq_config.flt3_B1 = 313631;
		eq_config.flt3_B2 = -423854;

		ret = bk_aud_driver_init();
		ret = bk_aud_dac_init(&dac_config);
		ret = bk_aud_eq_init(&eq_config);
		CLI_LOGI("init audio driver and dac successful\n");
		...
		dma_config.src.end_addr = (uint32_t)data_buffer + test_size;
		...
		bk_dma_set_transfer_len(DMA_ID_2, test_size);
		...
		while(1)
		{
			fr = f_read(&file, data_buffer, test_size, &uiTemp);
			if (fr != FR_OK) {
