...
extern int shell_log_forward(u8 level, char * log_buf, u16 log_len);

/* ===============================  internal functions  =========================== */
static void shell_ipc_rx_isr(u8 chnl_id, mb_chnl_cmd_t *cmd_buf)
{
	u32		result = 0;
	...
	if(cmd_buf->hdr.cmd == MB_CMD_LOG_OUT)
	{
		...
		if(log_cmd->len >= 2)
		{
			log_cmd->buf[log_cmd->len-2] = '\r';
			log_cmd->buf[log_cmd->len-1] = '\n';
		}
		
		shell_log_forward(CPU1_LOG_INFO, (char *)log_cmd->buf, log_cmd->len);  /* don't call registered rx-callback, copy data to log buffer directly. */
