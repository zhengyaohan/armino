...
#include <string.h>
#include "bk_driver_codec_es8374.h"
...
void aud_initial(uint32_t freq, uint32_t channels, uint32_t bits_per_sample)
{
	...
	sddev_control(DD_DEV_TYPE_I2S, I2S_CMD_SET_FREQ_DATAWIDTH, &rate);
...
int32_t aud_hw_init(void)
{
	...
	vTaskDelay(10);
	sddev_control(DD_DEV_TYPE_I2S, I2S_HW_SET, (void *)0);
...
void aud_open(void)
{
	...
	sddev_control(DD_DEV_TYPE_I2S, I2S_CMD_ACTIVE, (void *)&enable);
