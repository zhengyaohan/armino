...
#define log_ota(...)                     bk_printf("\033[36;22m");bk_printf(__VA_ARGS__);bk_printf("\033[0m\r\n")
...
int ota_erase_dl_rbl(void)
{
	...
	for (i = 0; i < part_table_size; i++) {
		...
		if (!strncmp(part_table[i].name, RT_BK_DL_PART_NAME, FAL_DEV_NAME_MAX)) {
			log_e("ota_erase_dl_rbl erase:%x\r\n", part_table[i].offset);
...
int ota_main(UINT32 *ex)
{
	...
	if (rt_ota_init() >= 0) {
		 ...
		 */
		log_d("check upgrade...");
		if ((result = rt_ota_check_upgrade()) == 1) {
			// need to upgrade
