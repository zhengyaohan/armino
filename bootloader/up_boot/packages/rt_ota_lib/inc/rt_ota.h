...
#define RT_OTA_SW_VERSION              "0.2.4"
...
#undef  log_d
#endif
#define log_d(...)                     bk_printf("[D/OTA] (%s:%d) ", __FUNCTION__, __LINE__);           bk_printf(__VA_ARGS__);bk_printf("\n")
...
#undef  log_e
#endif
#define log_e(...)                     bk_printf("\033[31;22m[E/OTA] (%s:%d) ", __FUNCTION__, __LINE__);bk_printf(__VA_ARGS__);bk_printf("\033[0m\n")

/* info level log */
#ifdef  log_i
#undef  log_i
#endif
#define log_i(...)                     bk_printf("\033[36;22m[I/OTA] ");                                bk_printf(__VA_ARGS__);bk_printf("\033[0m\n")
...
/**
 * verify firmware header on this partition
 *
 * @param part partition
 *
 * @return -1: failed, >=0: success
 */
int rt_ota_part_fw_verify_header(const struct fal_partition *part);
