...
void set_flash_protect(PROTECT_TYPE type)
{
    ...
    //bk_printf("set_flash_protect %x %x\r\n",type,param);
...
#if USE_CHIP_ENCRYPT
u32 no_crc[0x1100 >> 2];
u32 encrypt_crc[0x1100 >> 2];
u32 uart_write = 0;
u32 ota_over = 0;
u32 app_bin_start_addr = 0;
u32 app_bin_len = 0;

extern void encrypt(u32 *rx, uint8_t *tx, u32 num,u32  addr0);
...
{
    unsigned long i, j,reg_value;
    unsigned long addr = address & (~0x1f);
    unsigned long buf[8];
    unsigned char *pb = (unsigned char *)&buf[0];

#if (USE_CHIP_ENCRYPT && (CFG_OTA_TYPE == OTA_RTT))
    //printf("w:%x %x\r\n",address,len);
    if((uart_write == 0) && (address >= app_bin_start_addr) && (address < (app_bin_start_addr + app_bin_len)))
    {
        for(i=0,j=0;i<len;i++)
        {
            if(((i%34) != 33) && ((i%34) != 32) )
                ((unsigned char *)no_crc)[j++] = buffer[i];
            else
                ((unsigned char *)no_crc)[j++] = 0xff;
        }
        
        encrypt(no_crc, (uint8_t *)encrypt_crc, (len/34), address);
        calc_crc(encrypt_crc, (len/34));
        buffer = (unsigned char *)encrypt_crc;
        //if(len < 0x1100)
            {
            ota_over = 1;
        }
...
u32 flash_get_reg7_config(void)
{
    while (REG_FLASH_OPERATE_SW & FLASH_OPERATE_SW_BUSY_SW_MASK);
    return REG_FLASH_CONFIG;
}


void flash_set_reg7_config(u32 reg)
{
    while (REG_FLASH_OPERATE_SW & FLASH_OPERATE_SW_BUSY_SW_MASK);
    REG_FLASH_CONFIG = reg;
    while (REG_FLASH_OPERATE_SW & FLASH_OPERATE_SW_BUSY_SW_MASK);
}
