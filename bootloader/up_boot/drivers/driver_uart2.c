...
void uart2_init(unsigned long ulBaudRate)
{
    ...
    REG_UART2_FIFO_CONFIG = (1  << UART2_FIFO_CONFIG_TX_THRESHOLD_POSI)
                            | (8  << UART2_FIFO_CONFIG_RX_THRESHOLD_POSI)
...
void UART2_InterruptHandler(void)
{
    ...
    {
#if (UART_DOWNLOAD_PORT == DEBUG_PORT_UART2)
...
static uint8_t uart2_receive_byte(void)
{
    while (!UART2_RX_READ_READY);
