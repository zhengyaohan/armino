...
function(__build_set_default_build_specifications)
    ...
    if(NOT "${armino_target}" STREQUAL "bk7256" AND NOT "${armino_target}" STREQUAL "bk7256_cp1")
		list(APPEND compile_options "-mcpu=arm968e-s"
                                    "-march=armv5te"
                                    "-mthumb"
                                    "-mthumb-interwork"
                                    "-mlittle-endian"
        ...
        list(APPEND compile_options "-mstrict-align")
    endif()
...
function(__build_init armino_path)
    ...
    file(GLOB component_dirs ${armino_path}/components/*)
    foreach(component_dir ${component_dirs})
        ...
        if(EXISTS ${component_dir}/component.txt)
            file(GLOB l2_component_dirs ${component_dir}/*)
    ...
    endforeach()
    file(GLOB middleware_dirs ${armino_path}/middleware/*)
    foreach(middleware_dir ${middleware_dirs})
        get_filename_component(middleware_dir ${middleware_dir} ABSOLUTE)
        __component_dir_quick_check(is_component ${middleware_dir})
        if(is_component)
            __component_add(${middleware_dir} ${prefix})
        endif()
        if(EXISTS ${middleware_dir}/component.txt)
            file(GLOB l2_component_dirs ${middleware_dir}/*)
            foreach(l2_component_dir ${l2_component_dirs})
                if(EXISTS ${l2_component_dir}/CMakeLists.txt)
                    get_filename_component(base_dir ${l2_component_dir} NAME)
                    __component_dir_quick_check(is_component ${l2_component_dir})
    ...
    endforeach()
    #Please let that the include dir after the components and middleware dir,
    #otherwise the repeat component in include can not be checked out.
    #inclde dir has no grandchild dir(l2), otherwise prefix will be error in armino_build_component().
    armino_build_get_property(inc_prefix __INCLUDE_PREFIX)
    file(GLOB include_dirs ${armino_path}/include/*)
    foreach(include_dir ${include_dirs})
        get_filename_component(include_dir ${include_dir} ABSOLUTE)
        __component_dir_quick_check(is_component ${include_dir})
        if(is_component)
            __component_add(${include_dir} ${inc_prefix})
	endif()
    endforeach()

    # Add internal components to the build
    # armino_build_get_property(prefix __PREFIX)
    # file(GLOB component_dirs $ENV{OSK_PATH}/components/*)
    # foreach(component_dir ${component_dirs})
    #     get_filename_component(component_dir ${component_dir} ABSOLUTE)
    #     __component_dir_quick_check(is_component ${component_dir})
    #     if(is_component)
    ...
    set(requires_common bk_common bk_log bk_event driver bk_rtos common)
...
function(armino_build_executable bin)
    ...
    if("${target}" STREQUAL "bk7256" OR "${target}" STREQUAL "bk7256_cp1")
       ...
       add_custom_command(OUTPUT "${bin_dir}/bin_tmp"
    	...
    	set(armino_objcopy ${TOOLCHAIN_DIR}/arm-none-eabi-objcopy)
         add_custom_command(OUTPUT "${bin_dir}/bin_tmp"
        COMMAND "${armino_objcopy}" -O binary "${bin_dir}/${bin}" "${bin_dir}/${bin_name}.bin"
        COMMAND python "${armino_pack}" -n "all-${bin_name}.bin" -f "${bin_name}.bin" -c "${BDK_SOC}"
        DEPENDS ${bin}
